...
import { ProblemEntry, Step2Data, Step6Data, ChatSession, FrameworkState, Source } from './types';
import { INITIAL_STEP2, INITIAL_STEP6, AI_APPROACHES } from './constants';
...
// Labels mapping for Export/Import
  ...
  state.problems.forEach((p, idx) => {
    ...
    if (p.research && p.research.length > 0) {
        ...
        p.research.forEach(r => {
            ...
            // Add a specific delimiter for result content so it is easier to parse
            md += `> Result:\n\n${r.result}\n\n`;
        });
    }
...
export const parseMarkdownImport = (md: string): Partial<FrameworkState> => {
    ...
    // Define exact keys that are string fields in ProblemEntry to avoid type errors
    ...
    // Mapping for Multi-line text fields
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // --- 1. Detect New Hypothesis ---
        // Match "## Hypothesis: Title" or "## 💡 Hypothesis: Title"
        // IMPORTANT: We must ensure we are NOT inside a research block to accidentally trigger this
        ...
        // --- 2. Check for Start of a Text Field (Multi-line) ---
        // If we are capturing research, we ignore standard fields until research block ends
        if (!isCapturingResearch) {
            let foundNewField = false;
            for (const [key, prop] of Object.entries(TEXT_FIELDS)) {
                // Looking for line starting with "- **Problem:**"
        ...
        // --- 4. Check for Terminators (Things that END a text block) ---
        // If we hit "### 5. Risks", turn off research and capturing
        ...
        if (isCapturingResearch) {
            // Detect new research item start
            ...
            // Add content to current research item
            // Ignore sub-headers inside research text unless it matches a MAIN section header format (like ### 5. Risks)
            // We need to be careful not to treat markdown headers inside research text as App Headers
            if (currentResearchItem) {
                 // Check if this line looks like a main section header that should exit research mode
                 if (line.startsWith('### ') && !line.startsWith('#### ')) {
                     // This is likely a section exit, but we handle that in step 4 check above.
                     // However, if we missed it (e.g. malformed export), we rely on step 4 check order.
                 } else {
        ...
        const isHeader = line.startsWith('###'); // Any subsection header
        ...
        if (isHeader || isImpact || isTechnology || isStep2 || isStep6) {
            capturingField = null; // Stop capturing standard text field
        }

        // --- 5. Capture Multi-line Content (Standard Fields) ---
        if (capturingField) {
            // Append current line to the field
            if (line.trim() !== '') {
        ...
        // --- 6. Parse Single-line numeric/select fields ---
